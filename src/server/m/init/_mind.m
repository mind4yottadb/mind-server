...
start(params)
	...
	set %mindParams("uApi")=""                                          ; INTERNAL: JDOM of uApi file. AFTER LOGIN get re-merged to current file
	set %mindParams("uApiJson")=""                                      ; INTERNAL: JSON of uApi file (to be sent to clients)
	set %mindParams("uApiServer")=""                                    ; INTERNAL: uApi server configuration sub-leg "vars", "map","code" and "hooks",then file,
	set %mindParams("uApiDataTypes")=",string,int,float,boolean,object,null,varByRef,json," ; INTERNAL:
	set %mindParams("uApiPropsDataTypes")=",string,int,float,boolean,"  ; INTERNAL:
	set %mindParams("usersFile")="$ydb_dist/plugin/etc/mind/users.json" ; the file that contains users
	set %mindParams("zio")=$principal                                   ; INTERNAL: $principal
	...
	set %mindParams("lstats")=""                                        ; INTERNAL: holds the local statistics
	set %mindParams("errorDump")=1                                      ; 0: none 1: only $Zstatus, 2: full
	set %mindParams("initOnly")=0                                       ; if true, it will quit after login
	set %mindParams("serverInfo")=""                                    ; INTERNAL: get infos pre-populated, to speed up login
	set %mindParams("zroutines")=""                                     ; INTERNAL: original $zroutines, to restore after testing .so and M files
	set %mindParams("serverPid")=""                                     ; INTERNAL: process Id of the MIND server
	set %mindParams("idleTimeout")=30                                   ; number of MINUTES of inactivity on the socket before to suicide
	set %mindParams("idleTimeout","ut")=0                               ; INTERNAL: $zut/1E6, internally used by idleTimeout
	set %mindParams("idleTimeout","socketTimeout")=60                   ; INTERNAL: number of SECONDS to timeout and trigger the ticker
	set %mindParams("sigusr2")=0                                        ; INTERNAL: true if SIGUSR2 can be processed
	set %mindParams("pool","pids")=""                                   ; INTERNAL: only on devOps session: contains a list of the pool PIDs
