...
RUN apt-get update && apt-get install -y curl unzip wget cmake git gcc make \
			libasound2 libnss3-dev libgdk-pixbuf2.0-dev \
...
# Install lastest version of YDB Web Server (version in yottadb/octo is old)
RUN git clone https://gitlab.com/YottaDB/Util/YDB-Web-Server.git
RUN cd YDB-Web-Server && cmake -B build/ . && cmake --build build/ && cmake --install build/

ENV ydb_xc_libcurl="/opt/yottadb/current/plugin/libcurl.xc"

# Create Certificates
# See https://github.com/FiloSottile/mkcert/issues/219 for Firefox/Chrome support inside of Docker
# Old openssl code just in case it needs to get resurrected
