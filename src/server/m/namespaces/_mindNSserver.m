...
login
    ...
    ;
    ; verify mindParams
    if $zpiece(command(1),":",1)=""!($zpiece(command(1),":",2)="") set %mindRes="*2"_CRLF_"-MISSING CREDENTIAL(s)"_CRLF_"-username and/or password not provided"_CRLF,%mindRes("status")=0 goto loginQuit
    ;
    ; update driver info
...
; ************************************************************
kill
    if +$get(command(1))=0 set %mindRes="-the PID has not been provided"_CRLF,%mindRes("status")=0 quit
    if +$get(command(2))'=2,+$get(command(2))'=9 set %mindRes="-the signal number is not valid"_CRLF,%mindRes("status")=0 quit
    ;
